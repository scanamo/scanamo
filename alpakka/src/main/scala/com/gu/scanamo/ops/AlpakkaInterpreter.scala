...
object AlpakkaInterpreter {
      ...
      override def apply[A](ops: ScanamoOpsA[A]) =
        ops match {
          ...
          case ConditionalUpdate(req) =>
            client.single(JavaRequests.update(req).legacy)
  ...
  object AwsLegacyOps {
    ...
    implicit final def v1AttributesList(v2: java.util.Map[String, AttributeValue]) : java.util.Map[String, v1.AttributeValue] = {
      v2.asScala.mapValues(_.asLegacy).asJava
    }

    implicit final def v2AttributesList(v1List: java.util.Map[String, v1.AttributeValue]) : java.util.Map[String, AttributeValue] = {
    ...
    implicit final class PutResponseOps(req: v1.PutItemResult) {
      def fromLegacy: PutItemResponse = {
        PutItemResponse.builder().attributes(req.getAttributes)
          .consumedCapacity(req.getConsumedCapacity.fromLegacy)
          .itemCollectionMetrics(req.getItemCollectionMetrics).build()
      }
    }
    ...
    implicit final class ScanResponseOps(req: v1.ScanResult) {
      def fromLegacy: ScanResponse = {
        ScanResponse.builder().consumedCapacity(req.getConsumedCapacity.fromLegacy)
          .count(req.getCount)
...
//      new v1.WriteRequest(req.putRequest(), req.deleteRequest())
    ...
    implicit def v1PutRequest(v2: v1.PutItemRequest): PutItemRequest = {
      PutItemRequest.builder().tableName(v2.getTableName)
...
//        new v1.BatchGetItemRequest(req.requestItems(), req.returnConsumedCapacity())
    ...
    implicit final class BatchGetItemRespOps(req: v1.BatchGetItemResult) {
      def fromLegacy: BatchGetItemResponse = {
        ...
        req.getUnprocessedKeys.asScala.mapValues {
          k =>
        ...
        val a = BatchGetItemResponse.builder()
...
//          .keys(req.getKeys.asScala.map(_.asScala.mapValues(_.fromLegacy).asJava).asJava)
    ...
    implicit final class ExcpetoOps(req: v1.ConditionalCheckFailedException) {
      def fromLegacy: ConditionalCheckFailedException = {
        ...
        val sdkHttpResponse = SdkHttpResponse.builder()
          .statusText("")
        ...
        ConditionalCheckFailedException.builder()
          ...
          .build().asInstanceOf[ConditionalCheckFailedException]
      }
    }
