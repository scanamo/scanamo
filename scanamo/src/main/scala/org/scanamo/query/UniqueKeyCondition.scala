...
object UniqueKeyCondition {
  ...
  ): UniqueKeyCondition[AndEqualsCondition[H, R]] { type K = (KH, KR) } =
...
object UniqueKeyConditions {
  ...
  implicit def keyList[V: DynamoFormat]: UniqueKeyConditions[KeyList[V]] =
    (kl: KeyList[V]) => kl.values.map(v => DynamoObject(kl.key.placeholder("") -> v))

  implicit def multipleKeyList[H: DynamoFormat, R: DynamoFormat]: UniqueKeyConditions[MultipleKeyList[H, R]] =
    (mkl: MultipleKeyList[H, R]) => {
