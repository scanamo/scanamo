...
sealed abstract class DynamoArray extends Product with Serializable { self =>
  ...
  /**
    ...
    */
  final def contains(x: DynamoValue): Boolean = self match {
    case Empty          => false
  ...
  /**
    ...
    */
  final def isValueArray: Boolean = self match {
  ...
  /**
    ...
    */
  final def isNumericArray: Boolean = self match {
  ...
  final def asArray: Option[List[DynamoValue]] = self match {
    ...
    case StrictS(xs)    => Some(xs.stream.map[DynamoValue](DynamoValue.fromString(_)).reduce[List[DynamoValue]](Nil, _ :+ _, _ ++ _).reverse)
    case PureS(xs)      => Some(xs.map(DynamoValue.fromString))
    case StrictN(xs)    => Some(xs.stream.map[DynamoValue](DynamoValue.unsafeFromNumber(_)).reduce[List[DynamoValue]](Nil, _ :+ _, _ ++ _).reverse)
    case PureN(xs)      => Some(xs.map(DynamoValue.unsafeFromNumber))
    case StrictB(xs)    => Some(xs.stream.map[DynamoValue](DynamoValue.fromByteBuffer(_)).reduce[List[DynamoValue]](Nil, _ :+ _, _ ++ _).reverse)
  ...
  /**
    ...
    */
  final def asNumericArray: Option[List[String]] = self match {
    case Empty          => Some(List.empty)
