...
/**
  * Provides the same interface as [[com.gu.scanamo.Scanamo]], but requires a generic [[cats.effect.Effect]] type
...
trait EffectAsync {
  ...
  def update[F[_]: Effect, V: DynamoFormat](client: AmazonDynamoDBAsync)(tableName: String)(
    key: UniqueKey[_], expression: UpdateExpression): F[Either[DynamoReadError,V]] =
  ...
  def queryIndexWithLimit[F[_]: Effect, T: DynamoFormat](client: AmazonDynamoDBAsync)(tableName: String, indexName: String)(
