...
private[scanamo] trait DynamoResultStream[Req, Res] {
  ...
  def stream[T: DynamoFormat](req: Req): ScanamoOps[List[Either[DynamoReadError, T]]] = {
    ...
    def streamMore(
        lastKey: Option[EvaluationKey],
        remainingLimit: Option[Int]): ScanamoOps[List[Either[DynamoReadError, T]]] = {
      for {
        ...
        resultList <- Option(lastEvaluatedKey(queryResult))
          .filterNot(_ => resultsStillToGet.exists(_ <= 0))
          .foldLeft(
...
private[scanamo] object DynamoResultStream {
  object ScanResultStream extends DynamoResultStream[ScanamoScanRequest, ScanResult] {
    ...
    override def withExclusiveStartKey(
        req: ScanamoScanRequest,
        key: util.Map[String, AttributeValue]): ScanamoScanRequest =
